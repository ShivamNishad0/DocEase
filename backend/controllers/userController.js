...
const paymentCashfree = async (req, res) => {
    try {
        ...
        const orderPayload = {
            ...
            order_currency: process.env.CURRENCY || 'INR',
            customer_details: {
...
const verifyCashfree = async (req, res) => {
    try {
        ...
        if (orderStatus === 'PAID') {
            ...
            res.json({ success: false, message: `Payment status: ${orderStatus}` });
        }
    } catch (error) {
...
const verifyRazorpay = async (req, res) => {
    try {
        ...
        if (orderInfo.status === 'paid') {
            console.log('Payment successful, updating appointment:', orderInfo.receipt)
            await appointmentModel.findByIdAndUpdate(orderInfo.receipt, { payment: true })
            res.json({ success: true, message: "Payment Successful" })
        }
        else {
            console.log('Payment not paid, status:', orderInfo.status)
